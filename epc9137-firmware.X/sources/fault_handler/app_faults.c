...
#include "config/epc9137_r40_hwdescr.h"
...
volatile FAULT_OBJECT_t fltobj_BuckOTP;
...
volatile uint16_t appFaults_Initialize(void) 
{
    ...
    // Initialize OTP fault object

    fltobj_BuckOTP.source_obj = &buck.data.temp;   // Set pointer to variable to monitor
    fltobj_BuckOTP.ref_obj = NULL;      // Clear pointer to "compare against" variable 
    fltobj_BuckOTP.bit_mask = 0xFFFF;  // Compare all bits of SOURCE (no bit filter)
    
    fltobj_BuckOTP.status.bits.type = FLTCMP_GREATER_THAN; // Select Compare-Type
    fltobj_BuckOTP.counter = 0;        // Clear fault event counter
    fltobj_BuckOTP.trip_level = BUCK_TEMP_OTL;    // Set fault trip level
    fltobj_BuckOTP.tripcnt_max = 50;    // Set counter level at which a FAULT condition will be tripped
    fltobj_BuckOTP.reset_level = BUCK_TEMP_OTL_RELEASE;   // Set fault reset level
    fltobj_BuckOTP.rstcnt_max = 2000;     // Set counter level at which a FAULT condition will be cleared
    
    // User-function declarations
    fltobj_BuckOTP.trip_response = &appPowerSupply_Suspend; // Set pointer to user-function which should be called when a FAULT is tripped
    fltobj_BuckOTP.reset_response = &appPowerSupply_Resume; // Set pointer to user-function which should be called when a FAULT is cleared
    
    fltobj_BuckOTP.status.bits.fault_active = true; // Set fault condition flag (must be cleared by fault check)
    fltobj_BuckOTP.status.bits.fault_status = true; // Set fault flag (must be cleared by fault check)
    fltobj_BuckOTP.status.bits.enabled = true; // Enable fault checks           
...
volatile uint16_t appFaults_Execute(void) 
{
    ...
    fres &= fault_check(&fltobj_BuckOTP);
