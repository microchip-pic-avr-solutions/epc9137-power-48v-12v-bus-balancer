...
{ 
    ...
    ADEIELbits.EIEN1 = 1; // Early interrupt is enabled for the channel 1
...
{ 
    ...
    buck.status.bits.fault_active = (bool) (
        ...
        fltobj_BuckOTP.status.bits.fault_status    
        );
...
volatile uint16_t appPowerSupply_ConverterObjectInitialize(void)
{
    ...
    buck.gpio.PowerGood.enabled = false; // disable POWER GOOD
...
//    buck.gpio.PowerGood.io_type = 0;    // This pin is configured as OUTPUT
    ...
    BUCK_TEMP_ANSEL = (int)buck.feedback.ad_temp.enabled;
    ...
    buck.startup.v_ramp.ref_minimum = BUCK_ISNS_CMOD_LIMIT;
