...
volatile uint16_t drv_BuckConverter_Execute(volatile BUCK_POWER_CONTROLLER_t* buckInstance) {
    ...
    switch (buckInstance->mode) {
        ...
        case BUCK_STATE_LAUNCH_V_RAMP:
            ...
            // If current sense requires a minimum common mode voltage, enforce 
            // voltage reference to start from this value or higher
            if (buckInstance->startup.v_ramp.reference < buckInstance->startup.v_ramp.ref_minimum)
                buckInstance->startup.v_ramp.reference = buckInstance->startup.v_ramp.ref_minimum;                
            }
        ...
        case BUCK_STATE_V_RAMP_UP:
            ...
            if(!buckInstance->v_loop.controller->status.bits.enabled) {
                ...
                {
                    buckInstance->v_loop.controller->status.bits.enabled = true; // enable voltage loop controller
                    // if open loop startup is required to overcome current sense common mode voltage
                    // limits, set Open Loop Startup Flag and keep current loops disabled
                    if (buckInstance->data.v_out < buckInstance->startup.v_ramp.ref_minimum)
                    {
                        _open_loop_start = true; // set Open Loop Startup Flag
                    }
                    else
                    {
                        ...
                        _open_loop_start = false; // clear Open Loop Startup Flag
                    }                    
                ...
                // vloop already enabled
                // check if open loop startup is still required
                if (buckInstance->data.v_out < buckInstance->startup.v_ramp.ref_minimum)
                {
                    Nop();
                    _open_loop_start = true; // set Open Loop Startup Flag
                    for (_i=0; _i<buckInstance->set_values.phases; _i++) 
                    {
                        if ((buckInstance->i_loop[_i].controller->Ports.Target.ptrAddress != NULL) && 
                            (!buckInstance->i_loop[_i].controller->status.bits.swap_target))
                        {
                            buckInstance->i_loop[_i].controller->status.bits.enabled = false;
                            _write_dummy = *buckInstance->i_loop[_i].controller->Ports.Target.ptrAddress;
                            _write_dummy += 16;
                            *buckInstance->i_loop[_i].controller->Ports.Target.ptrAddress = _write_dummy; 
                            *buckInstance->i_loop[_i].controller->ADCTriggerControl.ptrADCTriggerARegister = BUCK_ISNS1_ADC_TRGDLY + (_write_dummy >> 1);
                        }
                        ...
                        {
                            buckInstance->i_loop[_i].controller->status.bits.enabled = false;
                            _write_dummy = *buckInstance->i_loop[_i].controller->Ports.AltTarget.ptrAddress;
                            _write_dummy += 16;
                            *buckInstance->i_loop[_i].controller->Ports.AltTarget.ptrAddress = _write_dummy; 
                        }
                ...
                else
                {
                    // enable all phase current loop controllers
                    for (_i=0; _i<buckInstance->set_values.phases; _i++) {
                        if(!buckInstance->i_loop[_i].controller->status.bits.enabled) {
                            // re-charge iloops
                            _write_dummy = *buckInstance->i_loop[_i].controller->Ports.Target.ptrAddress;
                            buckInstance->i_loop[_i].ctrl_Precharge(
                                buckInstance->i_loop[_i].controller, 0, _write_dummy
                            );
                            buckInstance->i_loop[_i].controller->status.bits.enabled = true; }
                    }
